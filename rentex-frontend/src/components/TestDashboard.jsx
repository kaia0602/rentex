...
const TestDashboard = () => {
    ...
    const [signupData, setSignupData] = useState({
        email: '', password: '', name: '', nickname: '', userType: 'USER',
        businessNo: '', contactEmail: '', contactPhone: ''
    });
    ...
    const handlePasswordResetRequest = async (e) => {
        e.preventDefault();
        try {
            const response = await apiClient.post('/api/auth/password-reset/request', { email: resetEmail });
    ...
    const handleUpdateProfile = async (e) => {
        ...
        const token = localStorage.getItem('accessToken');
        try {
            await apiClient.put('/api/users/me', { nickname: newNickname }, {
                headers: { 'Authorization': token }
            });
            alert('닉네임이 변경되었습니다.');
    ...
    const handleRequestReturn = async (rentalId) => {
        ...
        try {
            await apiClient.post(`/api/rentals/${rentalId}/return-request`, {}, {
                headers: { 'Authorization': token }
            });
            ...
            <div className="column">
                <section className="section">
                    ...
                    <form onSubmit={handleSignup} className="form-group">
                        ...
                        <input name="password" type="password" placeholder="비밀번호" onChange={handleSignupChange} required />
                        ...
                        {signupData.userType === 'PARTNER' && (
                            <>
                                <input name="businessNo" placeholder="사업자 번호" onChange={handleSignupChange} required />
                                <input name="contactEmail" placeholder="연락용 이메일" onChange={handleSignupChange} />
                                <input name="contactPhone" placeholder="연락용 전화번호" onChange={handleSignupChange} />
                            </>
                        )}
                ...
                <section className="section">
                    ...
                    <form onSubmit={handleLogin} className="form-group">
                        ...
                        <input name="password" type="password" placeholder="비밀번호" onChange={handleLoginChange} required />
                ...
                <section className="section">
                    ...
                    {resetStep === 1 ? (
                        ...
                        <form onSubmit={handlePasswordResetVerify} className="form-group">
                            ...
                            <button type="button" onClick={() => setResetStep(1)}>이메일 다시 입력</button>
                        </form>
                    )}
                </section>
            </div>
                ...
                <section className="section">
                    ...
                    <div className="api-test-group">
                        <p className="token-status">
                            현재 토큰: {storedToken ? <span className="token-saved">저장됨</span> : <span className="token-none">없음</span>}
                        </p>
                    ...
                    {apiResponse && apiResponse.email ? (
                        ...
                        <div className="my-page-view">
                            <div className="profile-section">
                                <h3>내 정보</h3>
                                <p><strong>이메일:</strong> {apiResponse.email}</p>
                                <p><strong>이름:</strong> {apiResponse.name}</p>
                                <p><strong>닉네임:</strong> {apiResponse.nickname}
                                    <button onClick={() => { setIsEditingNickname(!isEditingNickname); setNewNickname(apiResponse.nickname); }}>{isEditingNickname ? '취소' : '수정'}</button>
                                </p>
                                {isEditingNickname && (
                                    <form onSubmit={handleUpdateProfile} className="form-group">
                                        <input value={newNickname} onChange={(e) => setNewNickname(e.target.value)} required />
                                        <button type="submit">저장</button>
                                    </form>
                                )}
                                <p><strong>등급:</strong> {apiResponse.role}</p>
                                <p><strong>총 벌점:</strong> {apiResponse.penaltyPoints}</p>
                            </div>

                            <div className="history-section">
                                <h3>대여 내역</h3>
                                {apiResponse.rentalHistory?.length > 0 ? (
                                    <ul>{apiResponse.rentalHistory.map(r => (
                                        <li key={r.rentalId}>
                                            {r.itemName} ({r.startDate} ~ {r.endDate}) - <strong>{r.status}</strong>
                                            {r.status === 'RENTED' && <button onClick={() => handleRequestReturn(r.rentalId)}>반납 요청</button>}
                                        </li>
                                    ))}</ul>
                                ) : <p>대여 내역이 없습니다.</p>}
                            </div>
                            ...
                            <div className="history-section">
                                ...
                                {apiResponse.penaltyHistory?.length > 0 ? (
                                    <ul>{apiResponse.penaltyHistory.map((p, i) => (
                                        <li key={i}>{p.reason}: {p.points}점 (납부: {p.isPaid ? '완료' : '미납'})</li>
                                    ))}</ul>
                                ) : <p>벌점 내역이 없습니다.</p>}
                            </div>
